...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (document.querySelector('body').classList.contains('donations-donation_success')) {
  ...
  const sendPayment = (e) => {
    ...
    .then(function (response) {
        const spinner = document.querySelector('.loading .circle-loader')
        const checkmark = spinner.querySelector('.checkmark')

        spinner.classList.add('load-complete');
        checkmark.classList.add('show');

        setTimeout(() => { document.location.href = '/donate/success' }, 5000);
      })
